...
### Windows (ready)
- `RoboScoutAI-windows-x64.exe` — upload as-is
...
Download all `RoboScoutAI-linux-x86_64.tar.gz.part*` files, then:
```bash
cat RoboScoutAI-linux-x86_64.tar.gz.part* > RoboScoutAI-linux-x86_64.tar.gz
```
...
```
Upload `dist/release/RoboScoutAI-macos-arm64.zip`.
