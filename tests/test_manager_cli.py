...
def test_cli_version_and_list(root, tmp_path):
    ...
    data = _read(out)
    assert data["app_version"] == "0.6.0" and data["manager_version"] == "0.7.0"
...
def test_managed_endpoints_delegate_to_manager(root, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(managed, "run_manager", lambda args, timeout=0: {"available": True, "latest_version": "0.7.1", "message": "update available", "can_apply": True})
    spawned = []
    ...
    assert check["available"] and check["mode"] == "managed" and check["current_version"] == "0.7.0"
    ...
    ipc.write_status(root, "done", progress=100, version="0.7.1")
    status = client.get("/api/updates/status").json()
    assert status["state"] == "done" and status["version"] == "0.7.1" and status["managed"]
    exits = []
...
def test_managed_run_manager_uses_python_module_in_dev(root, monkeypatch):
    ...
    assert result["manager_version"] == "0.7.0" and result["root"] == str(root)
